...
pub fn trace_layer(
    ...
    if luma.width() as usize != w || luma.height() as usize != h {
        return Vec::new();
    }
    ...
    // 4 + 5. simplify + fit ---------------------------------------------
    let to_world = |x: f64, y: f64| Vec2::new(x * p.scale, (p.img_height as f64 - y) * p.scale);
    let mut out = Vec::new();
    for path in paths {
...
fn emit_arc(
    raw: &[(f64, f64)],
    simp: &[(f64, f64)],
    to_world: &impl Fn(f64, f64) -> Vec2,
    out: &mut Vec<Geom>,
) -> bool {
    if raw.len() < 5 {
        return false;
    }
    let Some((cx, cy, r)) = fit_circle(raw) else {
        return false;
    };
...
fn trace_polylines(bits: &[bool], w: usize, h: usize, min_run: usize) -> Vec<Vec<(usize, usize)>> {
    ...
    let at = |x: i32, y: i32| -> bool {
        x >= 0
            ...
            && (x as usize) < w
            && (y as usize) < h
            && bits[y as usize * w + x as usize]
    };
    let degree = |x: usize, y: usize| -> usize {
        N8.iter()
            .filter(|(dx, dy)| at(x as i32 + dx, y as i32 + dy))
            .count()
    };
    ...
    for &endpoints_first in &[true, false] {
        for y in 0..h {
            for x in 0..w {
                if !bits[y * w + x] || visited[y * w + x] {
                    continue;
                }
                let is_end = degree(x, y) <= 1;
                if endpoints_first && !is_end {
                    continue;
                }
...
mod tests {
    ...
    fn straight_ink_run_makes_a_line() {
        ...
        assert!(
            !g.is_empty(),
            "expected at least one line from a straight stroke"
        );
