...
fn main() {
    ...
    let Some(input) = a.get(1).cloned() else {
        eprintln!("usage: render_dxf <file> [out.png] [conv]");
        std::process::exit(1);
    };
    ...
    let dxf_path = if input.to_lowercase().ends_with(".dwg") {
        let conv = a.get(3).cloned().unwrap_or_else(|| {
            format!(
                "{}/.local/bin/dwgconv {{in}} {{out}}",
                std::env::var("HOME").unwrap_or_default()
            )
        });
        let tmp = std::env::temp_dir().join("render_dxf.dxf");
        let cmd = conv
            .replace("{in}", &format!("'{}'", input))
            .replace("{out}", &format!("'{}'", tmp.display()));
    ...
    let to_px = |p: Vec2| {
        (
            (pad + (p.x - mn.x) * s) as i32,
            (h as f64 - pad - (p.y - mn.y) * s) as i32,
        )
    };
...
fn emit(g: &Geom, doc: &Document, out: &mut Vec<Vec<Vec2>>) {
    ...
    match g {
        ...
        Geom::Arc(arc) => out.push(
            (0..=n)
                .map(|i| {
                    let t = arc.start_angle + (i as f64 / n as f64) * arc.sweep_angle;
                    Vec2::new(
        ...
        Geom::Ellipse(e) => out.push(
            (0..=n)
                .map(|i| e.point_at(i as f64 / n as f64 * TAU))
                .collect(),
        ),
        Geom::EllipseArc(ea) => out.push(
            (0..=n)
                .map(|i| {
                    ea.ellipse
                        .point_at(ea.start_param + (i as f64 / n as f64) * ea.sweep_param)
                })
                .collect(),
        ),
