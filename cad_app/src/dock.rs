...
use egui::{Align2, Color32, Context, CursorIcon, FontId, Id, Pos2, Rect, Sense, Stroke, Ui, Vec2};
...
pub trait DockHost {
    ...
    fn show(
        ...
        state: &mut DockState,
        open: &mut bool,
        body: impl FnOnce(&mut Ui, Option<f32>),
    ) -> Rect;
}
...
pub(crate) fn header_band(
    ...
    let (rect, band) = ui.allocate_exact_size(
        egui::vec2(w, crate::theme::space::HEADER_BAND),
    ...
    if !title.is_empty() {
        p.text(
            egui::pos2(rect.left() + edge, rect.center().y),
            Align2::LEFT_CENTER,
            title,
            crate::theme::typ::title(),
            TEXT,
        );
    }
    ...
    if closable {
        ...
        let xr = Rect::from_center_size(
            egui::pos2(right_x - 10.0, rect.center().y),
            egui::vec2(20.0, 20.0),
        );
        let over_x = ui.rect_contains_pointer(xr);
        if band.hovered() {
            ui.ctx().set_cursor_icon(if over_x {
    ...
    if let Some(b) = badge {
        ...
        let tw = ui
            .fonts(|f| f.layout_no_wrap(b.to_owned(), font.clone(), accent))
            .size()
            .x;
        let pw = tw + 16.0;
        let pill = Rect::from_min_size(
            egui::pos2(right_x - pw, rect.center().y - 9.0),
            egui::vec2(pw, 18.0),
        );
        p.rect(
            pill,
            egui::Rounding::same(9.0),
            Color32::from_rgba_unmultiplied(0x00, 0xe5, 0xff, 26),
            Stroke::NONE,
        );
...
fn rail_header_band(ui: &mut Ui, cfg: &DockConfig) -> (bool, egui::Response) {
    ...
    let w = ui.available_width();
    let (rect, band) =
        ui.allocate_exact_size(egui::vec2(w, RAIL_HEADER_H), Sense::click_and_drag());
    ...
    if cfg.collapsible {
        ...
        let tri = if collapsed {
            vec![
                egui::pos2(cc.x - s * 0.5, cc.y - s),
                egui::pos2(cc.x - s * 0.5, cc.y + s),
                egui::pos2(cc.x + s * 0.7, cc.y),
            ] // ▸
        } else {
            vec![
                egui::pos2(cc.x - s, cc.y - s * 0.5),
                egui::pos2(cc.x + s, cc.y - s * 0.5),
        ...
        if cresp.clicked() {
            ui.ctx().data_mut(|d| d.insert_temp(cid, !collapsed));
        }
    ...
    for _ in 0..5 {
        p.rect_filled(
            Rect::from_min_size(egui::pos2(grip_x, gy), egui::vec2(grip_w, 1.5)),
            0.0,
            grip_col,
        );
        gy += 3.0;
    ...
    let grip_r = Rect::from_min_size(
        egui::pos2(grip_x - 2.0, rect.top()),
        egui::vec2(grip_w + 4.0, RAIL_HEADER_H),
    );
    ...
    let xr = Rect::from_center_size(
        egui::pos2(grip_x - 16.0, rect.center().y),
        egui::vec2(18.0, 18.0),
    );
    let over_x = ui.rect_contains_pointer(xr);
    let xcol = if over_x { TEXT } else { MUTED };
    ...
    let mut menu_open = ui
        .ctx()
        .data(|d| d.get_temp::<bool>(menu_id).unwrap_or(false));
...
fn paint_close_flyout(ctx: &Context, id: Id, anchor: Pos2) -> Rect {
    ...
    let h = 26.0_f32;
    let tw = ctx
        .fonts(|f| f.layout_no_wrap(label.to_owned(), font.clone(), TEXT))
        .size()
        .x;
    let w = tw + pad_x * 2.0;
...
fn float_pos_for(cfg: &DockConfig, sr: egui::Rect, r: DockRegion) -> Pos2 {
    match r {
        ...
        DockRegion::Right => egui::pos2(
            (sr.right() - cfg.float_w - 60.0).max(20.0),
            sr.center().y - 120.0,
        ),
        DockRegion::Bottom => egui::pos2(
            sr.center().x - cfg.float_w * 0.5,
            (sr.bottom() - 240.0).max(sr.top() + 60.0),
        ),
        DockRegion::Top => egui::pos2(
            sr.center().x - cfg.float_w * 0.5,
            (sr.top() + 44.0).max(44.0),
        ),
    }
}
...
impl DockHost for EguiDockHost {
    fn show(
        ...
        let collapsed = cfg.collapsible
            && ctx.data(|d| {
                d.get_temp::<bool>(Id::new((cfg.id, "collapsed")))
                    .unwrap_or(false)
            });
            ...
            DockState::Docked(stored) => {
                ...
                let rect = match region {
                    ...
                    DockRegion::Bottom => {
                        egui::TopBottomPanel::bottom(Id::new((cfg.id, "dock")))
                            ...
                            .default_height(if cfg.any_edge {
                                cfg.strip_h.max(1.0)
                            } else {
                                cfg.size
                            })
                ...
                if let Some(p) = undock {
                    ...
                    let fp = match region {
                        ...
                        DockRegion::Bottom | DockRegion::Top => egui::pos2(
                            sr.center().x - cfg.float_w * 0.5,
                        ...
                        DockRegion::Left => egui::pos2(p.x.max(sr.left() + 60.0), p.y),
                    };
