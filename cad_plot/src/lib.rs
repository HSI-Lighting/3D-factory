...
impl std::fmt::Display for PlotError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PlotError::UnsupportedTarget => {
                write!(f, "system-printer output is not implemented (PDF/SVG only)")
            }
...
pub fn export_png_meta(
    ...
    let scene = build_scene(doc, table, cfg);
    let bytes = png::scene_to_png_bytes(&scene, dpi).map_err(PlotError::Pdf)?; // reuse Pdf variant for raster errors
    std::fs::write(path, &bytes).map_err(PlotError::Io)?;
    Ok(PlotOutcome {
        path: path.to_path_buf(),
        bytes: bytes.len(),
...
pub fn export_png_bytes(
    ...
    let scene = build_scene(doc, table, cfg);
    png::scene_to_png_bytes(&scene, dpi).map_err(PlotError::Pdf)
}
...
mod tests {
    ...
    fn lineweight_is_physical_across_scales() {
        ...
        let pf = match &fit.prims[0] {
            Prim::Stroke { pts, .. } => pts.clone(),
            _ => panic!(),
        };
        let ph = match &hundred.prims[0] {
            Prim::Stroke { pts, .. } => pts.clone(),
            _ => panic!(),
        };
    ...
    fn default_width_plots_thin_and_distinct_from_025() {
        ...
        let w0 = stroke_widths(&build_scene(
            &doc_with_aci1_line(Lineweight::Default),
            &table,
            &cfg(PlotScale::Fit),
        ));
        let w25 = stroke_widths(&build_scene(
            &doc_with_aci1_line(Lineweight::Custom(0.25)),
            &table,
            &cfg(PlotScale::Fit),
        ));
    ...
    fn window_clips_geometry_to_the_rect() {
        ...
        assert_eq!(
            outside.prims.len(),
            0,
            "a line outside the window is fully clipped"
        );
    }
    ...
    fn non_plottable_layer_is_skipped() {
        ...
        let scene = build_scene(&doc, &table, &cfg(PlotScale::Fit));
        assert!(
            stroke_widths(&scene).is_empty(),
            "non-plottable layer must not plot"
        );
    }
    ...
    fn layout_plot_maps_viewports_1to1_matching_layout_tab() {
        ...
        assert!(
            frame_found,
            "viewport frame must sit at its layout rect (no margin shift)"
        );
        ...
        assert!(
            model_mapped,
            "viewport model content must be centred on the viewport rect"
        );
    }
    ...
    fn pattern_hatch_emits_lines_not_a_solid_fill() {
        ...
        assert!(
            strokes.len() >= 3,
            "ANSI31 over a 20×20 square must add pattern lines, got {} strokes",
            strokes.len()
        );
        assert!(
            !scene.prims.iter().any(|p| matches!(p, Prim::Fill { .. })),
        ...
        assert!(
            scene.prims.iter().any(|p| matches!(
        ...
        assert!(
            (mx.0 - mn.0 - 20.0).abs() < 1e-6 && (mx.1 - mn.1 - 20.0).abs() < 1e-6,
    ...
    fn saved_ctb_cap_and_join_reach_the_scene() {
        ...
        assert_eq!(
            model,
            (EndStyle::Square, JoinStyle::Bevel),
        ...
        assert_eq!(
            model2,
            (EndStyle::Round, JoinStyle::Round),
    ...
    fn layout_plot_applies_saved_ctb_width_and_linetype() {
        ...
        doc.linetypes
            .linetypes
            .push(cad_kernel::linetype::Linetype::new("DASH", &[4.0, -2.0]));
        ...
        let model = scene
            ...
            .expect("model line stroke");
        assert!(
            (model.0 - 0.70).abs() < 1e-6,
            "CTB Fixed pen width must win over the object lineweight: {}",
            model.0
        );
        assert_eq!(
            model.1,
    ...
    fn layout_plot_clips_viewport_content_to_the_frame() {
        ...
        let line = scene
            ...
            .find_map(|p| match p {
                Prim::Stroke {
                    ...
                    width_mm,
                    ..
                } if *width_mm == 0.25 && pts.len() == 2 && (pts[0].1 - 40.0).abs() < 1e-9 => {
                    Some(pts.clone())
                }
                _ => None,
            })
        ...
        assert!(
            (line[0].0 - 30.0).abs() < 1e-6 && (line[0].1 - 40.0).abs() < 1e-6,
            "left endpoint must sit on the frame edge: {:?}",
            line
        );
        assert!(
            (line[1].0 - 70.0).abs() < 1e-6 && (line[1].1 - 40.0).abs() < 1e-6,
            "right endpoint must sit on the frame edge: {:?}",
            line
        );
    }
    ...
    fn polyline_merges_into_one_stroke_so_joins_apply() {
        ...
        use cad_kernel::geom::{PolyVertex, Polyline};
        let mut doc = Document::default();
        doc.push(DObject {
    ...
    fn builtin_ctb_viewports_do_not_leak_doc_plot_styles_widths() {
        ...
        let model = scene
            ...
            .expect("model line stroke");
        assert!(
            (model.0 - 0.25).abs() < 1e-6,
            "built-in CTB must keep the OBJECT width, not the doc table's 0.8: {}",
            model.0
        );
        assert!(
            model.1.is_empty(),
            "built-in CTB must keep the OBJECT linetype, not the doc table's dash: {:?}",
            model.1
        );
    }
    ...
    fn doc_with_hatch(aci: u8) -> Document {
        use cad_kernel::geom::{Hatch, HatchPattern, PolyVertex, Polyline};
        let mut doc = Document::default();
        let poly = Polyline {
            vertices: vec![
                PolyVertex {
    ...
    fn fill_style_horizontal_bars_emits_clipped_bar_strokes() {
        ...
        assert!(
            bars.len() >= 10,
            "expected ~10 horizontal bars (20 mm / 1.5), got {}",
            bars.len()
        );
                    ...
                    for &(x, y) in pts.iter() {
                        assert!(
                            x >= fx0 - 1e-6
                                ...
                                && y >= fy0 - 1e-6
                                && y <= fy1 + 1e-6,
                            "bars must be clipped to the polygon bbox: {:?}",
                            pts
                        );
                    }
                }
