...
fn tee_contact(p: Vec2, host: &Wall) -> Option<f64> {
    ...
    // handled separately.
    if along <= JOIN_TOL || along >= len - JOIN_TOL {
        return None;
    }
    let foot = a + d * u;
    if (p - foot).len() > host.thickness * 0.5 * T_BAND + JOIN_TOL {
        return None;
    }
    Some(u)
}
...
pub fn solve_faces(this: &Wall, all: &[Wall]) -> Option<WallFaces> {
    ...
    for (node, at_start) in [(this.start, true), (this.end, false)] {
        ...
        for n in all {
            if same_wall(this, n) || n.is_curved() {
                continue;
            }
            ...
            if at_s == at_e {
                continue;
            } // not incident, or degenerate both-ends
            let (Some(nl), Some(nr)) = (n.left_line(), n.right_line()) else {
                continue;
            };
            let out = if at_s {
                n.end - n.start
        ...
        if !others.is_empty() {
            ...
            let place = |m: Option<Vec2>,
                         concave: Vec2,
                         this_face: &cad_kernel::Line,
                         n_face: &cad_kernel::Line,
                         bevels: &mut Vec<(Vec2, Vec2)>|
             -> Option<Vec2> {
                let m = m?;
                let d = (m - node).len();
            ...
            if let Some(h) = all.iter().find(|h| {
                !same_wall(this, h)
                    && !h.is_curved()
                    && (h.start - node).len() >= JOIN_TOL
                    && (h.end - node).len() >= JOIN_TOL
                ...
                if hlen >= JOIN_TOL {
                    ...
                    let clip = |p: Vec2, face_pt: Vec2| -> Vec2 {
                        if inside(p) {
                            line_intersect(face_pt, dt, fa, fb - fa).unwrap_or(p)
                        } else {
                            p
                        }
                    };
...
pub fn solve_face_segments(
    ...
    for n in all {
        ...
        let (Some(near), Some(nl), Some(nr)) = (near, n.left_line(), n.right_line()) else {
            continue;
        };
...
mod tests {
    ...
    fn l_corner_90deg_miters_both_faces() {
        ...
        assert!(
            close(fa.left.0, Vec2::new(1.0, 1.0)),
            "inner miter, got {:?}",
            fa.left.0
        );
        assert!(
            close(fa.right.0, Vec2::new(-1.0, -1.0)),
            "outer miter, got {:?}",
            fa.right.0
        );
        // Far end untouched.
    ...
    fn l_corner_any_angle_meets_at_a_point() {
        ...
        assert!(
            b_ends.iter().any(|p| close(*p, a_inner)),
            "A inner {:?} not shared by B {:?}",
            a_inner,
            b_ends
        );
        assert!(
            b_ends.iter().any(|p| close(*p, a_outer)),
            "A outer {:?} not shared by B {:?}",
            a_outer,
            b_ends
        );
    }
    ...
    fn x_crossing_breaks_each_face_into_two_with_a_gap() {
        ...
        let b = Wall {
            start: Vec2::new(5.0, -10.0),
            end: Vec2::new(5.0, 10.0),
            thickness: 4.0,
            style: 0,
            bulge: 0.0,
        };
    ...
    fn t_junction_stem_trims_to_host_near_face() {
        ...
        assert!(
            close(f.left.0, Vec2::new(4.0, 2.0)),
            "left start trimmed to near face, got {:?}",
            f.left.0
        );
        assert!(
            close(f.right.0, Vec2::new(6.0, 2.0)),
            "right start trimmed to near face, got {:?}",
            f.right.0
        );
    ...
    fn t_junction_opening_is_stable_short_exact_and_deep() {
        ...
        for end_y in [3.0_f64, 2.0, 1.0] {
            // short of / on / past the near face
            let stem = Wall {
                start: Vec2::new(10.0, 8.0),
                end: Vec2::new(10.0, end_y),
                thickness: 2.0,
                style: 0,
                bulge: 0.0,
            };
            ...
            assert_eq!(
                l.len(),
                2,
                "near face must open (stem end y={end_y}), got {l:?}"
            );
            assert_eq!(r.len(), 1, "far face stays whole (stem end y={end_y})");
            assert!(
                (l[0].1.x - 9.0).abs() < 1e-6 && (l[1].0.x - 11.0).abs() < 1e-6,
    ...
    fn stem_ending_on_a_crossing_of_two_walls_butts_the_nearest() {
        ...
        for p in [f.left.1, f.right.1] {
            assert!(
                (p - node).len() <= 6.0,
                "stem face pokes far past the junction: {:?}",
                p
            );
        }
    }
    ...
    fn t_junction_stem_landing_on_the_face_still_fires() {
        ...
        let stem = Wall {
            start: Vec2::new(10.0, 2.5),
            end: Vec2::new(10.0, 12.0),
            thickness: 3.0,
            style: 0,
            bulge: 0.0,
        };
        ...
        assert!(
            (f.left.0.y - 2.5).abs() < 1e-6,
            "left not on face: {:?}",
            f.left.0
        );
        assert!(
            (f.right.0.y - 2.5).abs() < 1e-6,
            "right not on face: {:?}",
            f.right.0
        );
    ...
    fn t_junction_non_right_angle_trims_both_stem_faces() {
        ...
        assert!(
            (f.left.1.y - 2.0).abs() < 1e-6,
            "left face not trimmed to face: {:?}",
            f.left.1
        );
        assert!(
            (f.right.1.y - 2.0).abs() < 1e-6,
            "right face not trimmed to face: {:?}",
            f.right.1
        );
        let (l, r) = solve_face_segments(&host, &all).unwrap();
        assert_eq!(
            l.len(),
            2,
            "near face should open at a slanted T, got {l:?}"
        );
        assert_eq!(r.len(), 1, "far face should stay whole, got {r:?}");
    }
    ...
    fn corner_dropped_on_a_wall_span_butts_the_host() {
        ...
        let w13 = Wall {
            start: Vec2::new(35.0, 20.0),
            end: Vec2::new(20.0, 2.0),
            thickness: 2.0,
            style: 0,
            bulge: 0.0,
        };
        let w14 = Wall {
            start: Vec2::new(20.0, 2.0),
            end: Vec2::new(5.0, 20.0),
            thickness: 2.0,
            style: 0,
            bulge: 0.0,
        };
        ...
        assert!(
            l.len() >= 2,
            "host near face should open under the attached corner, got {l:?}"
        );
    }
    ...
    fn y_junction_three_walls_share_a_node_cleanly() {
        ...
        for (i, p) in pts.iter().enumerate() {
            let shared = pts
                .iter()
                .enumerate()
                .filter(|(j, q)| *j != i && close(*p, **q))
                .count();
            assert_eq!(
                shared, 1,
                "face end {i} {:?} not shared by exactly one other",
                p
            );
        }
    }
    ...
    fn moderately_sharp_corner_stays_a_true_miter() {
        ...
        let fb = solve_faces(&b, &all).unwrap();
        assert!(
            fa.bevels.is_empty() && fb.bevels.is_empty(),
            "30° corner must not bevel: {:?} {:?}",
            fa.bevels,
            fb.bevels
        );
        let b_ends = [fb.left.0, fb.right.0];
        for ae in [fa.left.1, fa.right.1] {
            assert!(
                b_ends.iter().any(|be| close(*be, ae)),
                "A end {:?} not shared by B {:?}",
                ae,
                b_ends
            );
        }
    ...
    fn sharp_fold_bevels_instead_of_spiking() {
        ...
        assert!(
            (fa.left.1 - node).len() <= bound,
            "left end still spikes: {:?}",
            fa.left.1
        );
        assert!(
            (fa.right.1 - node).len() <= bound,
            "right end still spikes: {:?}",
            fa.right.1
        );
        for (p, q) in &fa.bevels {
            assert!(
                (*p - node).len() <= bound && (*q - node).len() <= bound,
                "bevel far from node: {:?}",
                (p, q)
            );
        }
