...
pub fn plan(inp: &DoglegInput) -> Result<(DoglegMetrics, Vec<String>), ArchError> {
    if inp.going <= 0.0 {
        return Err(ArchError::NonPositive("going"));
    }
    if inp.width <= 0.0 {
        return Err(ArchError::NonPositive("width"));
    }
    if inp.total_rise <= 0.0 {
        return Err(ArchError::NonPositive("total_rise"));
    }
    if inp.well < 0.0 {
        return Err(ArchError::NonPositive("well (must be ≥ 0)"));
    }
    if inp.n_lower < 1 {
        return Err(ArchError::NonPositive("lower-flight steps"));
    }
    if inp.n_upper < 1 {
        return Err(ArchError::NonPositive("upper-flight steps"));
    }
    ...
    if inp.n_upper > inp.n_lower {
        return Err(ArchError::NonPositive(
            "upper flight longer than lower (swap the two, or reduce it)",
        ));
    }
    ...
    if WAIST_V <= riser {
        return Err(ArchError::NonPositive(
            "riser too tall for the soffit (waist must exceed the riser)",
        ));
    }
    ...
    if !(0.15..=0.20).contains(&riser) {
        warn.push(format!(
            "riser {:.3} m is outside the comfortable 0.15–0.20 m — building the spec exactly",
            riser
        ));
    }
    if !(0.60..=0.66).contains(&m.comfort_2r_t) {
        warn.push(format!(
            "2·riser + going = {:.3} m is outside the ideal 0.60–0.66 m",
            m.comfort_2r_t
        ));
    }
    if inp.going < 0.22 {
    ...
    warn.push(format!(
        "clear walking width ≈ {:.3} m (structural {:.3} m minus the finishes)",
        m.clear_width, inp.width
    ));
    warn.push(format!(
        "footprint {:.2} × {:.2} m — if it won't fit, change the step split, not the going",
        m.shaft_len, m.shaft_wid
    ));
    Ok((m, warn))
}
...
fn flight_profile(n: usize, going: f32, riser: f32, slope: f32, spring: Spring) -> Vec<[f32; 2]> {
    ...
    p.into_iter()
        .map(|[x, z]| [x + RISER_FACE_T, z - TREAD_THICK])
        .collect()
}
...
pub fn build(
    inp: &DoglegInput,
    with_treads: bool,
) -> Result<(DoglegMetrics, Vec<DoglegPart>), ArchError> {
    ...
    parts.push(DoglegPart::Flight {
        profile: pa,
        y0: a_y0,
        y1: a_y1,
        role: Role::FlightBody,
    });
...
mod tests {
    ...
    fn reference_metrics_match_spec() {
        ...
        assert!(
            (m.top_tread_z - 2.89).abs() < 1e-6,
            "top tread MUST be exact: {}",
            m.top_tread_z
        );
    ...
    fn flight_b_profile_matches_spec() {
        ...
        assert!(
            (p[0][0] - 0.015).abs() < 1e-4 && (p[0][1] - (-0.219583)).abs() < 1e-4,
            "v0 {:?}",
            p[0]
        );
        assert!(
            (p[1][0] - 0.015).abs() < 1e-4 && (p[1][1] - (-0.040)).abs() < 1e-4,
            "v1 {:?}",
            p[1]
        );
        assert!(
            (p[2][0] - 0.015).abs() < 1e-4 && (p[2][1] - 0.080417).abs() < 1e-4,
            "v2 {:?}",
            p[2]
        );
        assert!(
            (p[13][0] - 1.815).abs() < 1e-4 && (p[13][1] - 0.6825).abs() < 1e-4,
            "v13 {:?}",
            p[13]
        );
        assert!(
            (p[14][0] - 1.815).abs() < 1e-4 && (p[14][1] - 0.502917).abs() < 1e-4,
            "v14 {:?}",
            p[14]
        );
    }
    ...
    fn build_yields_editable_parts() {
        ...
        let flights = parts
            .iter()
            .filter(|p| p.role() == Role::FlightBody)
            .count();
        ...
        for p in &parts {
            match p {
                DoglegPart::Box { min, max, .. } => {
                    for k in 0..3 {
                        assert!(min[k].is_finite() && max[k].is_finite() && max[k] >= min[k]);
                    }
                    top = top.max(max[2]);
                }
                DoglegPart::Flight {
    ...
    fn dogleg_builds_a_valid_csg_solid() {
            ...
            match part {
                ...
                DoglegPart::Flight {
                    ...
                    let pts: Vec<glam::Vec2> = profile
                        ...
                        .collect();
                    let (prof, centre, w, d) = model
                        .add_profile(&pts)
                        .expect("flight sawtooth is a valid profile");
                    let plane = Plane::from_basis(
        ...
        assert!(
            (mx[2] - m.top_tread_z).abs() < 0.05,
            "reaches the floor height: {} vs {}",
            mx[2],
            m.top_tread_z
        );
        assert!(
            mx[0] - mn[0] > 5.0 && mx[1] - mn[1] > 2.0,
            "spans the {:.1}×{:.1} footprint",
            m.shaft_len,
            m.shaft_wid
        );
    }
