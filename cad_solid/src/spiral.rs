...
pub fn plan(inp: &SpiralInput) -> Result<(SpiralMetrics, Vec<String>), ArchError> {
    ...
    if inp.radius <= SLEEVE_R {
        return Err(ArchError::NonPositive(
            "radius (must clear the central pole)",
        ));
    }
    if inp.n_steps < 1 {
    ...
    if !(0.15..=0.20).contains(&riser) {
        warn.push(format!(
            "riser {:.3} m is outside the comfortable 0.15–0.20 m — building it as asked",
            riser
        ));
    }
    ...
    if m.going_walk < 0.22 {
        warn.push(format!(
            "going at the walk line ≈ {:.3} m is steep (< 0.22 m) — normal for a tight spiral",
            m.going_walk
        ));
    }
...
mod tests {
    ...
    fn reference_metrics_match_spec() {
        ...
        assert!(
            (m.step_angle_deg - (-17.142857)).abs() < 1e-4,
            "step angle {}",
            m.step_angle_deg
        );
        assert!(
            (m.pitch_m_per_deg - 0.00875).abs() < 1e-6,
            "pitch {}",
            m.pitch_m_per_deg
        );
        assert!(
            (m.total_rise - 3.150).abs() < 1e-4,
            "total rise {}",
            m.total_rise
        );
        assert!(
            (m.total_sweep_deg + 360.0).abs() < 1e-4,
            "sweep {}",
            m.total_sweep_deg
        );
    }
    ...
    fn every_rail_shares_the_tread_pitch() {
        ...
        assert!(
            (step_dz - m.riser).abs() < 1e-5,
            "one pitch: {} vs {}",
            step_dz,
            m.riser
        );
        // Consecutive handrail chords rise by exactly one riser.
        let hand: Vec<_> = parts
            .iter()
            .filter(|p| p.role() == Role::Handrail)
            .collect();
        assert!(hand.len() >= 2, "handrail is segmented");
        for p in &hand {
            if let SpiralPart::Seg { a, b, .. } = p {
                assert!(
                    (b[2] - a[2] - m.riser).abs() < 1e-4,
                    "each handrail chord climbs one riser"
                );
            }
        }
    }
    ...
    fn build_yields_editable_parts() {
        ...
        assert_eq!(
            count(Role::Handrail),
            20,
            "one chord between each pair of treads"
        );
        ...
        for p in &parts {
            match p {
                SpiralPart::Cyl {
                    ...
                    for v in [cx, cy, r, z0, z1] {
                        assert!(v.is_finite());
                    }
                    assert!(z1 >= z0);
                    overall_top = overall_top.max(*z1);
                    if *role == Role::Pole {
                        tread_top = tread_top.max(*z1);
                    }
                }
                SpiralPart::Prism {
                    poly, z0, z1, role, ..
                ...
                SpiralPart::Seg { a, b, r, .. } => {
                    for v in a.iter().chain(b).chain([r]) {
                        assert!(v.is_finite());
                    }
        ...
        assert!(
            (tread_top - inp.total_height).abs() < 0.05,
            "top tread/pole reaches {} m",
            inp.total_height
        );
        assert!(
            (overall_top - m.handrail_top_z).abs() < 0.05,
            "handrail top at {} m",
            m.handrail_top_z
        );
    }
    ...
    fn spiral_builds_a_valid_csg_solid() {
        ...
        for part in parts {
            match part {
                ...
                SpiralPart::Seg { a, b, r, sides, .. } => {
                    ...
                    model.push(
                        BoolOp::Union,
                        Plane::from_basis(a, u, v),
                        pl,
                        Primitive::Cylinder { r, h: len, sides },
                    );
                }
            }
        }
        ...
        assert!(
            (mx[2] - inp.total_height).abs() < 0.1,
            "reaches the floor height: {} vs {}",
            mx[2],
            inp.total_height
        );
        assert!(
            mx[0] - mn[0] > inp.radius,
            "spans at least the tread radius"
        );
    }

    #[test]
    fn rejects_bad_inputs() {
        assert!(plan(&SpiralInput {
            total_height: 0.0,
            ..Default::default()
        })
        .is_err());
    ...
    fn segment_basis_is_right_handed() {
        ...
        assert!(
            ...
            "u × v reproduces the segment direction"
        );
    }
}
