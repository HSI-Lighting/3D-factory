    ...
    match g {
        ...
        Geom::Arc(a) => {
            ...
            Feat {
                tag: 3,
                pts: vec![rel(s), rel(e)],
                scal: vec![(a.sweep_angle * 0.25).tan()],
            }
        }
        Geom::Point(pt) => Feat {
            tag: 4,
            pts: vec![rel(pt.location)],
            scal: vec![],
        },
...
pub fn diff_blocks(
    ...
    if index_ok {
        ...
        for i in 0..fa.len() {
            for j in 0..fb.len() {
                if used_b[j] || !same_shape(&fa[i], &fb[j]) {
                    continue;
                }
    ...
    out.clusters.sort_by(|x, y| {
        y.point_count.cmp(&x.point_count).then(
            y.magnitude
...
mod tests {
    use super::*;
    use crate::geom::{Line, PolyVertex, Polyline};
    ...
    fn closed_rectangle_right_edge_stretch() {
        let rect = |w: f64| {
            DObject::new(Geom::Polyline(Polyline {
                vertices: vec![
                    PolyVertex {
        ...
        let d = diff_blocks(&[rect(100.0)], Vec2::ZERO, &[rect(130.0)], Vec2::ZERO, 1e-6);
        assert_eq!(d.changed, 1);
        assert_eq!(d.clusters.len(), 1);
        let c = &d.clusters[0];
