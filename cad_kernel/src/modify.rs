...
use crate::geom::{
    Arc, Circle, Donut, Ellipse, Geom, Line, PolyVertex, Polyline, Ray, Spline, Wall, Xline,
};
    ...
    pub fn offset(&self, dist: f64, side: Vec2) -> Result<Geom, &'static str> {
        ...
        match self {
            ...
            Geom::Circle(c) => {
                ...
                if new_r <= EPS {
                    return Err("offset: would collapse to a point or smaller");
                }
                Ok(Geom::Circle(Circle {
                    center: c.center,
                    radius: new_r,
                }))
            }
            Geom::Arc(a) => {
                ...
                if new_r <= EPS {
                    return Err("offset: would collapse");
                }
                Ok(Geom::Arc(Arc {
                    center: a.center,
                    radius: new_r,
            ...
            Geom::Ellipse(el) => {
                ...
                let pts =
                    offset_ellipse_samples(*el, 0.0, std::f64::consts::TAU, dist, side, true)?;
                if pts.len() < 3 {
                    return Err("offset: ellipse degenerate");
                }
            ...
            Geom::EllipseArc(ea) => {
                ...
                let pts = offset_ellipse_samples(
                    ea.ellipse,
                    ea.start_param,
                    end_param,
                    dist,
                ...
                if pts.len() < 2 {
                    return Err("offset: ellipse arc degenerate");
                }
                Ok(Geom::Polyline(Polyline {
                    vertices: pts
                        .into_iter()
            ...
            Geom::Point(_) => Err("offset on point is undefined"),
            Geom::Hatch(_) => Err("offset on hatch is undefined (offset the boundary instead)"),
            Geom::Spline(s) => {
                ...
                if pts.len() < 2 {
                    return Err("offset: spline degenerate");
                }
                Ok(Geom::Polyline(Polyline {
                    vertices: pts
                        .into_iter()
            ...
            Geom::Xline(x) => {
                ...
                let n = x.dir.perp();
                let sgn = if (side - x.base).dot(n) >= 0.0 {
                    1.0
                } else {
                    -1.0
                };
            ...
            Geom::Ray(r) => {
                ...
                let n = r.dir.perp();
                let sgn = if (side - r.base).dot(n) >= 0.0 {
                    1.0
                } else {
                    -1.0
                };
            ...
            Geom::Donut(d) => {
                ...
                let sgn = if (side - d.center).dot(Vec2::new(1.0, 0.0)) >= 0.0 {
                    1.0
                } else {
                    -1.0
                };
            ...
            Geom::Wipeout(_) => Err("offset: a wipeout mask cannot be offset"),
            Geom::Region(_) => Err("offset: a region cannot be offset — use a polyline"),
            Geom::Wall(w) => {
                ...
                if let Geom::Line(l) = g {
                    ...
                    Err("offset wall: unexpected non-Line result")
                }
            }
            Geom::Table(_) => Err("offset on a table is undefined"),
            Geom::Xref(_) => Err("offset: explode the xref first"),
            Geom::Text(_) => Err("offset on text is undefined"),
            Geom::Leader(_) => Err("offset on leader is undefined"),
            Geom::CenterMark(_) => Err("offset on a center mark is undefined"),
            Geom::AttrDef(_) => Err("offset on attribute definition is undefined"),
            Geom::Dimension(_) => Err("offset on dimension is undefined"),
            Geom::BlockRef(_) => Err("offset: explode the block first"),
            Geom::Viewport(_) => Err("offset: viewport is a paper-space entity"),
        }
    }
}
...
fn offset_polyline(p: &Polyline, dist: f64, side: Vec2) -> Result<Geom, &'static str> {
    ...
    let n = v.len();
    if n < 2 {
        return Err("offset: polyline needs ≥ 2 vertices");
    }
    ...
    if (nb - na).len() < EPS {
        return Err("offset: zero-length polyline segment");
    }
    let left = (nb - na).perp().normalized();
    let mid = (na + nb) * 0.5;
    let hand = if (side - mid).dot(left) >= 0.0 {
        1.0
    } else {
        -1.0
    };
    ...
    for i in 0..seg_count {
        ...
        if bulge.abs() < 1e-9 {
            ...
            if rp <= EPS {
                return Err("offset: arc segment collapses");
            }
            let scale = rp / r;
            segs.push(OffSeg {
                a: c + (a - c) * scale,
    ...
    let join = |s0: &OffSeg, s1: &OffSeg| -> Vec2 {
        if s0.line && s1.line {
            if let Some(p) = line_line_inf(s0.a, s0.dir, s1.a, s1.dir) {
                return p;
            }
        }
        (s0.b + s1.a) * 0.5
    ...
    if p.closed {
        ...
        for i in 1..seg_count {
            out.push(PolyVertex {
                pos: join(&segs[i - 1], &segs[i]),
                bulge: segs[i].bulge,
            });
        }
        out.push(PolyVertex {
            pos: segs[seg_count - 1].b,
            bulge: 0.0,
        });
    }
    ...
    if p.closed && out.len() == seg_count && seg_count >= 3 {
        ...
        for i in 0..seg_count {
            ...
            if src_dir.len() > EPS && out_dir.len() > EPS && out_dir.dot(src_dir) < 0.0 {
        ...
        if seg_count <= 512 {
            ...
            for i in 0..seg_count {
                let Some((a0, a1)) = line_edge(i) else {
                    continue;
                };
                for j in (i + 2)..seg_count {
                    ...
                    let Some((b0, b1)) = line_edge(j) else {
                        continue;
                    };
...
fn offset_ellipse_samples(
    ...
    let n0u = nrm0 / nl;
    let sgn = if (side - p0).dot(n0u) >= 0.0 {
        1.0
    } else {
        -1.0
    };
    ...
    for i in 0..count {
        ...
        if inward {
            ...
            if amt > rho - 1e-9 {
                return Err("offset beyond the curve's radius of curvature — reduce the value");
            }
        }
        let p = el.point_at(t);
...
fn offset_spline_samples(sp: &Spline, dist: f64, side: Vec2) -> Vec<Vec2> {
    ...
    let n = (96.0 + span.log10().max(0.0) * 48.0)
        .round()
        .clamp(96.0, 512.0) as usize;
    ...
    let sgn = if (side - pts[0]).dot(n0 / nl0) >= 0.0 {
        1.0
    } else {
        -1.0
    };
...
pub fn fillet_lines(
    ...
    if radius < 0.0 {
        return Err("fillet: radius must be ≥ 0");
    }
    let d1v = l1.b - l1.a;
    let d2v = l2.b - l2.a;
    let len1 = d1v.len();
    ...
    if len1 < EPS || len2 < EPS {
        return Err("fillet: zero-length line");
    }

    // Infinite-line intersection.
    let cross = d1v.x * d2v.y - d1v.y * d2v.x;
    if cross.abs() < EPS {
        return Err("fillet: lines are parallel");
    }
    ...
    let endpoint_of_l1 = if (l1.a - i_pt).dot(dir1) > (l1.b - i_pt).dot(dir1) {
        l1.a
    } else {
        l1.b
    };
    let endpoint_of_l2 = if (l2.a - i_pt).dot(dir2) > (l2.b - i_pt).dot(dir2) {
        l2.a
    } else {
        l2.b
    };
    ...
    if radius < EPS {
        ...
        return Ok(FilletOut {
            g1_new: Geom::Line(Line {
                a: endpoint_of_l1,
                b: i_pt,
            }),
            g2_new: Geom::Line(Line {
                a: i_pt,
                b: endpoint_of_l2,
            }),
            arc: None,
    ...
    let start_angle = if d_ccw <= std::f64::consts::PI {
        a1
    } else {
        a2
    };
...
pub fn chamfer_lines(
    ...
    if d1 < 0.0 || d2 < 0.0 {
        return Err("chamfer: distances must be ≥ 0");
    }
    let d1v = l1.b - l1.a;
    let d2v = l2.b - l2.a;
    let len1 = d1v.len();
    let len2 = d2v.len();
    if len1 < EPS || len2 < EPS {
        return Err("chamfer: zero-length line");
    }

    let cross = d1v.x * d2v.y - d1v.y * d2v.x;
    if cross.abs() < EPS {
        return Err("chamfer: lines are parallel");
    }
    let dx = l2.a.x - l1.a.x;
    let dy = l2.a.y - l1.a.y;
    let t1 = (dx * d2v.y - dy * d2v.x) / cross;
