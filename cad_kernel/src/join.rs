...
pub fn bulge_from_arc(start: Vec2, end: Vec2, center: Vec2, sweep_abs: f64) -> f64 {
    ...
    let sign = if circ(ccw, mag) <= circ(ccw, tau - mag) {
        1.0
    } else {
        -1.0
    };
...
pub fn arc_from_bulge(a: Vec2, b: Vec2, bulge: f64) -> Option<(Vec2, f64, f64, f64)> {
    ...
    if theta.abs() >= std::f64::consts::TAU - 1e-9 {
        return None;
    } // full circle
    let half = theta * 0.5;
...
pub fn join_geoms(geoms: &[(usize, Geom)]) -> JoinOut {
    ...
    loop {
        ...
        let line_geoms: Vec<Line> = group
            .iter()
            .filter_map(|&i| match &available[i].1 {
                Geom::Line(l) => Some(*l),
                _ => None,
            })
            .collect();
        if let Some(merged_line) = merge_collinear_lines(&line_geoms) {
            for &local_i in &group {
                consumed.push(available[local_i].0);
            }
    ...
    loop {
        ...
        if let Some(pl) = chain_to_polyline(
            &chain
                .iter()
                .map(|&i| available[i].1.clone())
                .collect::<Vec<_>>(),
        ) {
            for &local_i in &chain {
                consumed.push(available[local_i].0);
            }
...
fn find_collinear_line_group(items: &[(usize, Geom)]) -> Vec<usize> {
    ...
    for i in 0..items.len() {
        let li = if let Geom::Line(l) = &items[i].1 {
            *l
        } else {
            continue;
        };
        ...
        for j in 0..items.len() {
            let lj = if let Geom::Line(l) = &items[j].1 {
                *l
            } else {
                continue;
            };
            ...
            if cross.abs() > JOIN_EPS * len_j {
                continue;
            }
            if (lj.a - li.a).dot(perp).abs() > JOIN_EPS {
                continue;
            }
...
fn find_concentric_arc_group(items: &[(usize, Geom)]) -> Vec<usize> {
    for i in 0..items.len() {
        let ai = if let Geom::Arc(a) = &items[i].1 {
            *a
        } else {
            continue;
        };
        ...
        for j in (i + 1)..items.len() {
            let aj = if let Geom::Arc(a) = &items[j].1 {
                *a
            } else {
                continue;
            };
            if (aj.center - ai.center).len() > JOIN_EPS {
                continue;
            }
            if (aj.radius - ai.radius).abs() > JOIN_EPS {
                continue;
            }
            group.push(j);
        }
...
fn arc_intervals(arcs: &[Arc]) -> Vec<(f64, f64)> {
    arcs.iter()
        .map(|a| {
            (
                a.start_angle.rem_euclid(std::f64::consts::TAU),
                a.sweep_angle,
            )
        })
        .collect()
...
fn find_touching_chain(items: &[(usize, Geom)]) -> Vec<usize> {
    ...
    for start in 0..items.len() {
        if endpoints_of(&items[start].1).is_none() {
            continue;
        }
        ...
        while let Some(cur) = queue.pop() {
            let Some((ca, cb)) = endpoints_of(&items[cur].1) else {
                continue;
            };
            for j in 0..items.len() {
                if seen[j] {
                ...
                let Some((ja, jb)) = endpoints_of(&items[j].1) else {
                    continue;
                };
                let touches = ca.dist(ja) < CHAIN_EPS
                    || ca.dist(jb) < CHAIN_EPS
                    || cb.dist(ja) < CHAIN_EPS
                    || cb.dist(jb) < CHAIN_EPS;
                if touches {
                    seen[j] = true;
                    queue.push(j);
                    group.push(j);
...
fn chain_to_polyline(geoms: &[Geom]) -> Option<Polyline> {
    ...
    for g in &remaining {
        let Some((a, b)) = endpoints_of(g) else {
            return None;
        };
    ...
    while !remaining.is_empty() {
        ...
        for (i, g) in remaining.iter().enumerate() {
            ...
            if a.dist(current) < CHAIN_EPS {
                found = Some(i);
                reverse = false;
                break;
            }
            if b.dist(current) < CHAIN_EPS {
                found = Some(i);
                reverse = true;
                break;
            }
...
mod concentric_arc_join_tests {
    ...
    fn wrap_around_chain_is_recognised_and_merged() {
        ...
        assert!(
            (deg(m.start_angle) - 350.0).abs() < 1e-4,
            "start {}",
            deg(m.start_angle)
        );
        assert!(
            (deg(m.sweep_angle) - 40.0).abs() < 1e-4,
            "sweep {}",
            deg(m.sweep_angle)
        );
    }
    ...
    fn overlapping_arcs_summing_past_tau_are_not_a_full_circle() {
        ...
        let m = merge_concentric_arcs(&arcs).unwrap();
        assert!(
            (deg(m.sweep_angle) - 300.0).abs() < 1e-4,
            "overlap covers 300°, got {}",
            deg(m.sweep_angle)
        );
    ...
    fn disjoint_arcs_with_a_gap_are_not_a_chain() {
        ...
        assert!(
            merge_concentric_arcs(&arcs).is_none(),
    ...
    fn adjacent_non_wrapping_arcs_still_chain() {
        ...
        let m = merge_concentric_arcs(&arcs).unwrap();
        assert!(
            (deg(m.start_angle) - 0.0).abs() < 1e-4,
            "start {}",
            deg(m.start_angle)
        );
        assert!(
            (deg(m.sweep_angle) - 60.0).abs() < 1e-4,
            "sweep {}",
            deg(m.sweep_angle)
        );
    }
    ...
    fn nested_overlap_merges_to_the_outer_span() {
        ...
        assert!(
            (deg(m.sweep_angle) - 300.0).abs() < 1e-4,
            "nested union is the outer 300° span, got {}",
            deg(m.sweep_angle)
        );
    }
    ...
    fn nested_triple_with_two_inners_is_a_single_cover() {
        ...
        assert!(
            (deg(m.sweep_angle) - 300.0).abs() < 1e-4,
            "merged span is the outer 300°, got {}",
            deg(m.sweep_angle)
        );
    }
}
