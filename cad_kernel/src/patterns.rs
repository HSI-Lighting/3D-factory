...
pub fn lookup(name: &str) -> Pattern {
    ...
    match up.as_str() {
        // ANSI31 — 45° diagonals  / / / / /
        "ANSI31" => Pattern::Families(vec![LineFamily {
            angle: pi / 4.0,
            base_x: 0.0,
            base_y: 0.0,
            spacing: 3.175,
        ...
        "ANSI33" => Pattern::Families(vec![LineFamily {
            angle: 3.0 * pi / 4.0,
            base_x: 0.0,
            base_y: 0.0,
            spacing: 3.175,
        ...
        "LINE" | "HORIZONTAL" => Pattern::Families(vec![LineFamily {
            angle: 0.0,
            base_x: 0.0,
            base_y: 0.0,
            spacing: 3.175,
...
mod tests {
    ...
    fn every_named_pattern_resolves() {
        for name in PATTERN_NAMES {
            if *name == "SOLID" {
                continue;
            } // SOLID is the no-line case
            let pat = lookup(name);
            assert!(
                !pat.is_empty(),
                "pattern '{}' resolved to empty pattern",
                name
            );
            match pat {
                Pattern::Families(fams) => {
                    for f in &fams {
                        assert!(
                            f.spacing > 0.0,
                            "pattern '{}' has non-positive spacing",
                            name
                        );
                    }
                }
                Pattern::Tile {
                    ...
                    assert!(
                        period_x > 0.0 && period_y > 0.0,
                        "pattern '{}' has non-positive period",
                        name
                    );
                    assert!(
                        !segments.is_empty() || !circles.is_empty(),
                        "pattern '{}' tile has no segments or circles",
                        name
                    );
                }
            }
    ...
    fn vertex_double_count_collapses_within_the_loop() {
        ...
        assert_eq!(
            intervals.len(),
            1,
            "full chord through the circle: {intervals:?}"
        );
        let span = intervals[0].1 - intervals[0].0;
        assert!(
            (span - 60.0).abs() < 1e-6,
    ...
    fn nested_island_xors_out_of_the_fill() {
        ...
        assert!(
            (a.1 - a.0 - 3.0).abs() < 1e-6 && (b.1 - b.0 - 3.0).abs() < 1e-6,
    ...
    fn disjoint_regions_stay_separate() {
        ...
        assert!(
            intervals[1].0 - intervals[0].1 > 5.0,
