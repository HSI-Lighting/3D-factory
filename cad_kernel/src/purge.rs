...
pub fn purge(doc: &mut Document) -> PurgeReport {
    ...
    for d in all_dobjects(doc) {
        let s = &d.style;
        if (s.layer as usize) < n_layers {
            used_layers[s.layer as usize] = true;
        }
        if (s.linetype as usize) < n_lt {
            used_lt[s.linetype as usize] = true;
        }
        match &d.geom {
            Geom::Text(t) => {
                if (t.style as usize) < n_ts {
                    used_ts[t.style as usize] = true;
                }
            }
            Geom::AttrDef(a) => {
                if (a.style as usize) < n_ts {
                    used_ts[a.style as usize] = true;
                }
            }
            Geom::Leader(l) => {
                if (l.label.style as usize) < n_ts {
                    used_ts[l.label.style as usize] = true;
                }
            }
            Geom::Dimension(dm) => {
                if (dm.style as usize) < n_ds {
                    used_ds[dm.style as usize] = true;
                }
            }
            Geom::Wall(w) => {
                if (w.style as usize) < n_ws {
                    used_ws[w.style as usize] = true;
                }
            }
            Geom::BlockRef(br) => {
                if (br.block as usize) < n_blk {
                    used_blk[br.block as usize] = true;
                }
            }
            _ => {}
        }
    ...
    for vp in doc.layouts.iter().flat_map(|l| l.viewports.iter()) {
        for &f in &vp.frozen_layers {
            if (f as usize) < n_layers {
                used_layers[f as usize] = true;
            }
        }
    }
    ...
    let drop_layer: Vec<usize> = (1..n_layers).filter(|&i| !used_layers[i]).collect();
        ...
        for d in doc
            ...
            .chain(
                doc.blocks
                    .blocks
                    .iter_mut()
                    .flat_map(|b| b.dobjects.iter_mut()),
            )
        {
...
mod tests {
    ...
    fn removes_unreferenced_layer_and_block() {
        ...
        let unused = doc.layers.add(crate::layer::Layer {
            name: "UnusedLayer".into(),
        ...
        let used = doc.layers.add(crate::layer::Layer {
            name: "UsedLayer".into(),
            color: crate::color::Color::Aci(7),
            linetype: 0,
            lineweight: crate::lineweight::Lineweight::Default,
        ...
        let mut d = DObject::new(Geom::Line(Line {
            a: Vec2::ZERO,
            b: Vec2::new(1.0, 0.0),
        }));
        d.style.layer = used;
        ...
        let d = doc
            .dobjects
            .iter()
            .find(|d| matches!(d.geom, Geom::Line(_)))
            .unwrap();
        ...
        let br = doc
            .dobjects
            .iter()
            .find(|d| matches!(d.geom, Geom::BlockRef(_)))
            .unwrap();
        if let Geom::BlockRef(b) = &br.geom {
            assert_eq!(
                doc.blocks.get(b.block).map(|x| x.name.as_str()),
                Some("UsedBlock")
            );
        } else {
            unreachable!();
    ...
    fn empty_doc_purges_nothing_but_removes_nothing() {
        ...
        assert!(
            r.is_empty(),
            "standard linetypes + reserved ids stay: {r:?}"
        );
    ...
    fn unused_style_removal_keeps_dobject_ids_valid() {
        ...
        assert!(
            doc.text_styles.get(s1).is_some(),
            "used style kept at its id"
        );
        assert_eq!(
            doc.text_styles.get(s1).map(|s| s.name.as_str()),
            Some("Keep")
        );
    }
}
